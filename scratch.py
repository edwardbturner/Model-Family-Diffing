...
def analyze_token_predictions(batch_idx, pos_idx, top_k=10):
    ...
    df = pd.concat(
        [df[df["is_correct"]], df[~df["is_correct"]].sort_values("big_logprob", ascending=False)]
...
if "big_actual_rank" in results_df.columns and "smol_actual_rank" in results_df.columns:
    filtered_df = results_df[
        (results_df["big_actual_rank"] == 1) & (results_df["smol_actual_rank"] > 50)
    ].copy()  # Use .copy() to avoid SettingWithCopyWarning if modifying later

    print(
        "Found {} tokens where the actual next token is rank 1 for Big model "
        "but >50 for Small model.".format(len(filtered_df))
    )
